...
int main(int argc, char **argv)
{
    ...
    {
		//szInputFile = sdkFindFilePath("testIn.jpg", argv[0]);
		szInputFile = "testIn.jpg";
    }
    ...
    ***************************/
    ...
    // Inverse DCT,将JPEG图像进行解码转化为YUV数据,YUV数据存储在apSrcImage[0],apSrcImage[1],apSrcImage[2]中
	//其步长(通道宽度)分别存在aSrcImageStep[0],aSrcImageStep[1],aSrcImageStep[2]中
    for (int i = 0; i < 3; ++ i)
    {
    ...
    ***************************/
	...
	cout << "nMCUBlocksV: " << nMCUBlocksV << endl;
	cout << "nMCUBlocksH: " << nMCUBlocksV << endl;
    for (int i=0; i < oFrameHeader.nComponents; ++i)
    {
        NppiSize oBlocks;
        NppiSize oBlocksPerMCU = {oFrameHeader.aSamplingFactors[i] >> 4, oFrameHeader.aSamplingFactors[i] & 0x0f};
